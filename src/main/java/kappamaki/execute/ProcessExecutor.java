...
public class ProcessExecutor implements ScenarioExecutor {
	 ...
	 * 
	 * @param index
	 *            An Index containing the scenario to run
	 * @param projectRoot
	 *            The root directory of a cuke4duke project
	 */
	...
	private void copyFeature(File tempDir, IndexedFeature feature) {
		try {
			...
			System.out.println("Copying " + feature.getUri() + " into "
					+ featureFile.getPath());
	...
	private void copyScenarios(File tempDir, IndexedScenario scenario, int line) {
		try {
			...
			Formatter formatter = new ExampleFilterer(new ScenarioTagger(
					scenario, tempFile), line);
			Parser parser = new Parser(formatter);
			System.out.println("Tagging " + feature.getUri() + " into "
					+ featureFile.getPath());
			parser.parse(gherkin, featureFile.getAbsolutePath(), 0);
	...
	private int executeScenarios(File tempDir, final int taskId) {
		...
		final File output = joinPaths(Utils.TEMP_DIR, "kappamaki",
				Integer.toString(taskId), "output");
		System.out.println("Executing: "
				+ Joiner.on(' ').join(projectRoot, mvn, features, tags, phase,
			...
			Future<String> submittedTask = executorService
					.submit(new Callable<String>() {
						...
						public String call() throws Exception {
							...
							String gherkin = Utils.readContents(output
									.getAbsolutePath());
	...
	public String getOutput(int taskId) {
		try {
			...
			throw new RuntimeException(
					"Unable to get resulting gherkin string", e);
		}
	}
	 ...
	 * 
	 * @param mvn
	 *            The maven executable path/name
	 */
	 ...
	 * 
	 * @param phase
	 *            the new Maven phase to execute
	 */
